...
b = 0.01;     % Zond aperture radius, m (set 0.1 for infinitely small zond)

lam = a/10;     % Wavelength in free space
R0 = 8*a^2/lam; % Reference distance
R = 10*R0/2;    % Distance from analyzed aperture to zond

Th = (0:0.1:10)*pi/180; % Theta angle. Can be set to one value (result will be a time field at zond output)
                        % or to array (result will be 2D RP at a given distance and frequency, and
                        % errors in RP compared with far field pattern)
r = 0:0.01:b;        % Integration parameter r
