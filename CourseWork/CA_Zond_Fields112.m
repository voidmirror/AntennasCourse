...
tic
% Design parameters
a = 4.;     % Analyzed aperture radius, m
b = 0.1;     % Zond aperture radius, m

lam = a/10;     % Wavelength in free space
R0 = 8*a^2/lam; % Reference distance
R = 100; %10*R0/1;    % Distance from analyzed aperture to zond


Th = 0.; % Theta angle. Can be set to one value (result will be a time field on zond output)
...
T = 10/c*lam;       % Time interval for FFT
...
% hold on;
%for j = 1:N_Th
    ro = R*sin(Th);   % Vector R projection to aperture plane, m
    z = R*cos(Th);     % Distance from observation point to aperture plane, m
    zb = z - r'*sin(phi)*sin(Th);
    rob = ro + r'*sin(phi)*cos(Th);
    for i = 1:N_phi
        E_e = zeros(N_r, N_i);
        for k = 1:N_r
